 ...
 * and the term is pretty SEO free while tabul (tabulify cli) is not
 */
public class Tabul {
    ...
    public static void main(String[] args) {
        ...
        rootCommand.addChildCommand(VERSION_COMMAND)
                .setDescription("Show build and version information");


        /**
         * Parse
         * May throw a {@link com.tabulify.cli.HelpPrintedException}
         */
        CliParser cliParser;
        try {
            ...
            try {
                ...
                if (cliParser.getBoolean(VERSION_FLAG)) {
                    ...
                    try {
                        ...
                        System.out.println("---------");
                        System.out.println("No version available because the class is not in a jar and no manifest is available.");
                        System.out.println("---------");
                        Tabul.exit(tabular, 1, null);
                    }
                    // for the compiler
         ...
         */
                ...
                for (CliCommand childCommand : commands) {
                    ...
                    switch (childCommand.getName()) {
                        ...
                        case VERSION_COMMAND:
                            feedbackDataPaths = TabulVersion.run(tabular, childCommand);
                            break;
                        default:
                            throw new IllegalCommandException("The sub-command (" + childCommand.getName() + ") is unknown for the command (" + CliUsage.getFullChainOfCommand(rootCommand) + ")");
                    }
                    ;
                ...
                if (e instanceof IllegalArgumentException) {
                    ...
                    /**
                     * Arguments parsing error or missing command
                     */
                    String originErrorFileName = e.getStackTrace()[0].getFileName();
                    if (e instanceof IllegalCommandException || (originErrorFileName != null && originErrorFileName.contains("CliParser"))) {
    ...
    /**
     * The first cli version was to return all downstream data path at once to print them
     * but with the advent of stream pipeline, we want to print in a streaming fashion
     * (ie as soon as we receive a downstream)
     * Because not all cli implements it, it's just a hack to return the data paths only
     * if there is no transfer
     * This function should be deleted when all cli create their own pipeline
     */
    public static List<DataPath> hackToNotPrintTwice(CliParser cliParser, List<DataPath> dataPaths) {
        /**
         * downstream data path should be loaded
         */
        String outputDataUriOption = cliParser.getString(OUTPUT_DATA_URI);
        if (outputDataUriOption != null) {
            return dataPaths;
