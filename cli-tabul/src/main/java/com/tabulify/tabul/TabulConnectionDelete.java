 ...
 * <p>
 */
public class TabulConnectionDelete {
  ...
  public static List<DataPath> run(Tabular tabular, CliCommand childCommand) {
    ...
    DataPath feedbackDataPath = tabular.getMemoryConnection().getDataPath("deletedConnection")
      .setComment("The connection(s) deleted")
    ...
    try (
      ...
      for (String name : names) {
        Set<KeyNormalizer> deletedConnections = connectionVault.deleteConnectionByGlobName(name);
        if (deletedConnections.isEmpty()) {
          ...
          if (tabular.isStrictExecution()) {
            ...
            LOGGER_TABUL.fine(msg);
          }
          continue;
        }
        for (KeyNormalizer deletedConnection : deletedConnections) {
          LOGGER_TABUL.fine("The connection (" + deletedConnection + ") was removed");
