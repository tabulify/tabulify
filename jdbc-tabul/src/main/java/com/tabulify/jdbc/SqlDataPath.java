...
public class SqlDataPath extends DataPathAbs {
  ...
  protected SqlDataPath(SqlConnection sqlConnection, String compactPath, DataPath executableDataPath, MediaType mediaType) {
    ...
    /**
     * The media type can be given or not
     * (Example: {@link SqlMediaType#REQUEST} or {@link SqlMediaType#RESULT_SET}
     * are always known
     */
    ...
    this.sqlConnectionResourcePath = SqlConnectionResourcePath.createOfConnectionPath(sqlConnection, compactPath, sqlMediaType);

    /**
     * After the resource path because we determine the type
     * with the path
     */
    if (this.mediaType == null || this.mediaType == OBJECT) {
