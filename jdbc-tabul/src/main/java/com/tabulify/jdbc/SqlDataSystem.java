...
public class SqlDataSystem extends DataSystemAbs {
  ...
  public List<SqlDataPath> select(DataPath dataPath, String globNameOrPath, MediaType mediaType) {
    ...
    SqlMediaType sqlMediaType = null;
    if (mediaType instanceof SqlMediaType) {
      sqlMediaType = (SqlMediaType) mediaType;
    }
    SqlConnectionResourcePath sqlResourcePath = SqlConnectionResourcePath.createOfConnectionPath(this.getConnection(), globNameOrPath, sqlMediaType);
      ...
      case SCHEMA:
        ...
        if (escapeCharacter == null) {
          ...
          if (!schemaPattern.containsSqlMatchers() &&
            ...
            // we retrieve all schemas and filter afterward
  ...
  /**
   * @param name - the sql name to validate
   *                throw if not valid
   */
  @Override
  public String validateName(String name) throws CastException {

    char firstChar = name.charAt(0);
    if (!String.valueOf(firstChar).matches("[a-zA-Z]")) {
      throw new CastException("Name (" + name + ") is not valid for sql as it should start with a Latin letter (a-z, A-Z), not " + firstChar);
    }

    if (!name.matches("[a-zA-Z0-9_]*")) {
      throw new CastException("Name (" + name + ") is not valid for sql. It should contain only the following characters (a-zA-Z0-9_)");
    }
