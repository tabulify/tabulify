...
public abstract class DataStoreProvider {
  ...
  // installed providers
  private static volatile List<DataStoreProvider> installedDataStoreProviders;
  ...
  private DataStoreProvider(Void ignore) {
  }
   ...
   */
  private static List<DataStoreProvider> loadInstalledProviders() {

    List<DataStoreProvider> dataStoreProviders = new ArrayList<>();

    ServiceLoader<DataStoreProvider> loadedTableSystemProviders = ServiceLoader
      .load(DataStoreProvider.class, ClassLoader.getSystemClassLoader());

    for (DataStoreProvider provider : loadedTableSystemProviders) {
      dataStoreProviders.add(provider);
    }

    return dataStoreProviders;
  }
   ...
   */
  public static List<DataStoreProvider> installedProviders() {
    if (installedDataStoreProviders == null) {
      ...
      synchronized (lock) {
        if (installedDataStoreProviders == null) {
          ...
          List<DataStoreProvider> list = AccessController
            .doPrivileged((PrivilegedAction<List<DataStoreProvider>>) () -> loadInstalledProviders());

          installedDataStoreProviders = Collections.unmodifiableList(list);
        }
      }
    }
    return installedDataStoreProviders;
  }
