...
public enum CryptoSymmetricCipher {
  ...
  public String encrypt(String passphrase, String plaintext) {
    ...
    byte[] cipherSalt = BytesUtil.getRandomBytes(saltLength);
    ...
    return Base64Util.toBase64(cipherSalt) + BASE64_SEP +
      Base64Util.toBase64(hmacDigest) + BASE64_SEP +
      Base64Util.toBase64(cipherText);
  ...
  public String decrypt(String passphrase, String storedCipherText) {
    ...
    byte[] saltBytes = Base64Util.toBytesFromBase64(blocks[0]);
    byte[] hmacBytes = Base64Util.toBytesFromBase64(blocks[1]);
    byte[] cipherText = Base64Util.toBytesFromBase64(blocks[2]);
    ...
    if (!(BytesUtil.equals(hmacDigest, hmacBytes))) {
