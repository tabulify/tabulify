 ...
 *
 * This script shows how class Loading is handled with the groovy.bat executable.
 * As the groovy executable don't add any other library in the classpath than the %GROOVY_HOME%\lib\groovy-X.X.X.jar
 * The function of the static class Paths will not work as its function depends on the defaultClassLoader that checks the classPath variable.
 *
 * Call this script:
 *   - your IDE
 *   - the %GROOVY_HOME%\bin\groovy(.bat) executable
 * Ans see the differences
 *
 *
 * The calling of the script through an IDEA works because they had all library in the classpath.
 *
 * Example:
...
println();
println("Load Service with Groovy Class Loader: "+Thread.currentThread().getContextClassLoader());
ServiceLoader<FileSystemProvider> loader = ServiceLoader.load(FileSystemProvider.class);
// Is the same as ServiceLoader<FileSystemProvider> loader = ServiceLoader.load(FileSystemProvider.class, Thread.currentThread().getContextClassLoader());
