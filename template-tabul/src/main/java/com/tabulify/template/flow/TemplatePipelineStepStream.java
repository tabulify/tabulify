...
import static com.tabulify.template.flow.TemplateEngine.NATIVE;
import static com.tabulify.template.flow.TemplateEngine.THYMELEAF;
import static com.tabulify.template.flow.TemplatePipelineStep.TEMPLATE;
import static com.tabulify.type.MediaTypeExtension.JSON_EXTENSION;
import static com.tabulify.type.MediaTypeExtension.TEXT_EXTENSION;
                    ...
                    if (!modelVariables.isEmpty()) {
                        ...
                        for (Map.Entry<String, DataUriNode> tablesVariable : modelVariables.entrySet()) {
                            ...
                            for (DataPath dataPath : dataPaths) {
                                try (SelectStream variableTableSelectStream = dataPath.getSelectStreamSafe()) {
                                    List<ColumnDef<?>> columnDefs = variableTableSelectStream.getRuntimeRelationDef().getColumnDefs();
                                    if (columnDefs.isEmpty()) {
                                        throw new InternalException("The data resource (" + dataPath + ") should have a column definitions. We can't read the records to add them to the model variables");
                                    }
