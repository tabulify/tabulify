...
import com.tabulify.text.yaml.YamlCast;
...
import static com.tabulify.template.flow.TemplateAttribute.*;
...
public class TemplatePipelineStep extends PipelineStepBuilderTarget {
    ...
    public TemplatePipelineStep addTemplateModelVariable(String name, DataUriNode dataPathUri) {
        this.modelVariables.put(name, dataPathUri);
        return this;
    }
    ...
    public TemplatePipelineStep setArgument(KeyNormalizer key, Object value) {
        ...
        switch (templateAttribute) {
            ...
            case MODEL_VARIABLES:
                ...
                List<Object> modelVariables = Casts.castToNewListSafe(value, Object.class);
                int modelVariableIndex = 0;
                for (Object modelVariable : modelVariables) {
                    modelVariableIndex++;
                    if (!(modelVariable instanceof Map)) {
                        throw new IllegalStateException("A table variable value is not a map but a " + value.getClass().getSimpleName());
                    }
                    Map<String, String> modelVariableMap;
                    try {
                        modelVariableMap = Casts.castToSameMap(modelVariable, String.class, String.class);
                    } catch (CastException e) {
                        throw new InternalException("String, string should not throw an exception", e);
                    }
                    String nameProperty = "name";
                    String variableName = modelVariableMap.get(nameProperty);
                    if (variableName == null) {
                        throw new IllegalArgumentException("The model variable " + modelVariableIndex + " should have a " + nameProperty + " property");
                    }
                    String dataUriProperty = "data-uri";
                    String dataUriValue = modelVariableMap.get(dataUriProperty);
                    if (dataUriValue == null) {
                        throw new IllegalArgumentException("The model variable " + modelVariableIndex + " should have a " + dataUriProperty + " property");
                    }
                    DataUriStringNode dataUriNode;
                    try {
                        dataUriNode = DataUriStringNode.createFromString(dataUriValue);
                    } catch (CastException e) {
                        throw new IllegalArgumentException("The model variable " + modelVariableIndex + " has a data uri value (" + dataUriValue + ") that is not valid. Message: " + e.getMessage(), e);
                    }
                    this.addTemplateModelVariable(variableName, getPipelineBuilder().getDataUri(dataUriNode));
                }
                break;
            ...
            case TEMPLATE_EMAIL:
                ...
                break;
            case TARGET_COLUMN_NAME:
                if (!(value instanceof String)) {
                    throw new IllegalStateException("The template attribute (" + templateAttribute + ")  is not a string but a " + value.getClass().getSimpleName());
                }
                this.setTargetColumnName(value.toString());
                break;
            case TARGET_TYPE:
                if (!(value instanceof String)) {
                    throw new IllegalStateException("The template attribute (" + templateAttribute + ")  is not a string but a " + value.getClass().getSimpleName());
                ...
                try {
                    this.setTargetType(Casts.cast(value.toString(), TemplateTargetType.class));
                } catch (CastException e) {
                    throw IllegalArgumentExceptions.createForArgumentValueForStep(
                            value.toString(),
                            templateAttribute,
                            this.toString(),
                            TemplateTargetType.class,
                            e);
                }
                break;
            case TEMPLATE_ENGINE:
                ...
                try {
                    this.setTemplateEngine(Casts.cast(value.toString(), TemplateEngine.class));
                } catch (CastException e) {
                    throw IllegalArgumentExceptions.createForArgumentValueForStep(
                            value.toString(),
                            templateAttribute,
    ...
    public PipelineStepBuilder setTargetColumnName(String targetColumnName) {
