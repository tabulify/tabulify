...
public class FillPipelineStepBatch extends PipelineStepIntermediateManyToManyAbs {
  ...
  public PipelineStepSupplierDataPath get() {
     ...
     */
    DataGenerator.DataGeneratorBuilder dataGeneratorBuilder = DataGenerator.create(getTabular());
    ...
    if (monoFillOperation) {
      ...
      dataGeneratorBuilder.addTransfer(genDataPath, acceptedTargetDataPaths.iterator().next());
    } else {
      // multi fill
      ...
      for (DataPath targetDataPath : acceptedTargetDataPaths) {
        ...
        if (sourceDataPath != null) {
          ...
          dataGeneratorBuilder.addTransfer(sourceDataPath, targetDataPath);
        } else {
          // no match by name
          ...
          dataGeneratorBuilder.addDummyTransfer(targetDataPath, maxRecordCount);
        }
      }
    }

    DataGenerator dataGenerator = dataGeneratorBuilder.build();
    Map<DataPath, GenDataPath> targetSourceMap = dataGenerator
      .getSourceTarget()
      .entrySet()
      .stream()
     ...
     * Conserve the order
     */
    List<GenDataPath> genDataPaths = new ArrayList<>();
    for (DataPath targetDataPath : acceptedTargetDataPaths) {
      genDataPaths.add(targetSourceMap.get(targetDataPath));
    }


    return (PipelineStepSupplierDataPath) DefinePipelineStep
      .builder()
      .addDataPaths(genDataPaths)
