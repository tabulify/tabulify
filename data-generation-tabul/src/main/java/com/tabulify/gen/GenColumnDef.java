...
import com.tabulify.type.Arrayss;
...
public class GenColumnDef<T> extends ColumnDefBase<T> implements ColumnDef<T> {
  ...
  private final GenRelationDef relationDataDef;
  ...
  private GenColumnDef(RelationDef relationDef, String columnName, SqlDataType<T> sqlDataType) {
    ...
    if (!(relationDef instanceof GenRelationDef)) {
      throw new IllegalStateException("The relation def (" + relationDef + ") is not a generator relation def");
    }
    this.relationDataDef = (GenRelationDef) relationDef;
    this.addVariablesFromEnumAttributeClass(GenColumnAttribute.class);
  }

  @Override
  public GenRelationDef getRelationDef() {
    return this.relationDataDef;
  }
  ...
  public CollectionGenerator<T> getGenerator() {
