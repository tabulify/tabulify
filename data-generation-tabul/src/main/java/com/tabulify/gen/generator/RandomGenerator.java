...
import com.tabulify.type.BigDecimals;
import com.tabulify.type.Casts;
import com.tabulify.type.Doubles;
import com.tabulify.type.Integers;
...
public class RandomGenerator<T> extends CollectionGeneratorAbs<T> implements CollectionGeneratorScale<T>, CollectionGenerator<T>, java.util.function.Supplier<T> {
  ...
  public RandomGenerator(Class<T> aClass, Object min, Object max) {
    ...
    if (Double.class.equals(aClass) || Float.class.equals(aClass)) {
      ...
      this.min = (min != null ? com.tabulify.type.time.Date.createFromObjectSafeCast(min).toSqlDate() : clazz.cast(minDefault));
      this.max = (max != null ? com.tabulify.type.time.Date.createFromObjectSafeCast(max).toSqlDate() : clazz.cast(maxDefault));
      ...
      this.min = (min != null ? com.tabulify.type.time.Timestamp.createFromObjectSafeCast(min).toSqlTimestamp() : clazz.cast(minTimestampDefault));
      this.max = (max != null ? com.tabulify.type.time.Timestamp.createFromObjectSafeCast(max).toSqlTimestamp() : clazz.cast(maxTimeStampDefault));
      ...
      this.min = (min != null ? com.tabulify.type.time.Time.createFromObject(min).toSqlTime() : clazz.cast(minTimeDefault));
      this.max = (max != null ? com.tabulify.type.time.Time.createFromObject(max).toSqlTime() : clazz.cast(maxTimeDefault));
