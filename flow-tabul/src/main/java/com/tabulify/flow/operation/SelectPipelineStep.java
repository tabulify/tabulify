...
public class SelectPipelineStep extends PipelineStepBuilderStreamSupplier {
    ...
    public SelectPipelineStep setArgument(KeyNormalizer key, Object value) {
        ...
        try {
            selectArgument = SelectPipelineStepArgument.cast(key);
        ...
        switch (selectArgument) {
            ...
            case MEDIA_TYPE:
                try {
                    this.setMediaType(MediaTypes.parse(attribute.getValueOrDefaultAsStringNotNull()));
                } catch (NullValueException e) {
                    throw new IllegalArgumentException("The argument (" + key + ") for the step (" + this + ") has a null value. Error: " + e.getMessage(), e);
                }
                break;
            default:
    ...
    public List<DataPath> produceDataPath() {
        ...
        if (!this.isPatternSelection() && !withDependencies) {
             ...
             * Note that because there is no glob expression, we may preserve the entry order ?
             * If asked as feature, we need a new {@link SelectPipelineStepArgumentOrder order}
             */
            return dataPaths
                    .stream()
                    .sorted()
