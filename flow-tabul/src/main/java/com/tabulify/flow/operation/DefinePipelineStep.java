        ...
        private void buildProcessArgumentDataResource(MapKeyIndependent<Object> dataResource) {
            ...
            for (Map.Entry<String, Object> entryDataResource : dataResource.entrySet()) {
                ...
                switch (definePipelineOperationAttribute) {
                    ...
                    case DATA_RECORDS:
                        if (value == null) {
                            if (this.getPipelineBuilder().isStrict()) {
                    ...
                    case DATA_URI:
                        ...
                        try {
                            dataUriStringNode = DataUriStringNode.createFromString(String.valueOf(value));
                        } catch (CastException e) {
                            throw new IllegalArgumentException("The value of the argument " + DefinePipelineStepResourceArgument.DATA_URI + " of the step (" + this + ") is not a valid data uri. Error: " + e.getMessage(), e);
                        }
                        dataUri = this.getPipelineBuilder().getDataUri(dataUriStringNode);
                        break;
                    case MEDIA_TYPE:
                        try {
            ...
            // Without a data uri (ie without a connection), this is an inline/in-pipeline memory resource
            if (mediaType != null) {
                /**
                 * Fragment data path
                 * This is a file because for now the file system handles fragment
                 * We don't use empty name because it's the default connection directory
                 */
                String resourcePath = Fs.getFileNameWithoutExtension(this.getPipelineBuilder().getScriptPath()) + "-define-" + (this.dataPaths.size() + 1) + "." + mediaType.getKind() + "-fragment";
                String resourceDataUri = resourcePath + "@" + ConnectionBuiltIn.TEMP_LOCAL_FILE_SYSTEM;
                dataPath = this.getTabular().getDataPath(resourceDataUri, mediaType);
                this.dataPaths.add(dataPath);
                if (dataDef != null) {
                    dataPath.mergeDataDefinitionFromYamlMap(dataDef);
                }
