...
public class IpVerticle extends AbstractVerticle {
      ...
      .onSuccess(configAccessor -> {
        ...
        vertx.executeBlocking(() -> Server
          ...
          .onSuccess(server -> {
            ...
            vertx.executeBlocking(() -> ipApp.mount())
              .onFailure(err -> this.handlePromiseFailure(verticlePromise, err))
  ...
  /**
   * Used in test to get a reference to the app
   * to create test request
   */
  public IpApp getIpApp() {
    if (this.ipApp == null) {
      throw new InternalException("The verticle should be first deployed");
    }
    return this.ipApp;
  }
}
