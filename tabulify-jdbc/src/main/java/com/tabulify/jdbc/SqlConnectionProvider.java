...
public abstract class SqlConnectionProvider {
  ...
  // installed providers
  private static volatile List<SqlConnectionProvider> installedProviders;
  ...
  private SqlConnectionProvider(Void ignore) {
  }
  ...
  private static List<SqlConnectionProvider> loadInstalledProviders() {

    List<SqlConnectionProvider> list = new ArrayList<>();

    ServiceLoader<SqlConnectionProvider> jdbcDataStoreExtensionProviders = ServiceLoader
      .load(SqlConnectionProvider.class, ClassLoader.getSystemClassLoader());

    for (SqlConnectionProvider provider : jdbcDataStoreExtensionProviders) {
        list.add(provider);
    }
   ...
   */
  public static List<SqlConnectionProvider> installedProviders() {
    if (installedProviders == null) {
      ...
      synchronized (lock) {
        if (installedProviders == null) {
          ...
          List<SqlConnectionProvider> list = AccessController
            .doPrivileged((PrivilegedAction<List<SqlConnectionProvider>>) SqlConnectionProvider::loadInstalledProviders);
   ...
   */
  public abstract Connection createSqlConnection(Tabular tabular, Attribute name, Attribute uri);
   ...
   * @return zero or more how to connections
   */
  ...
  /**
   * @param tabular - the context
   * @return zero or more how to service
   */
