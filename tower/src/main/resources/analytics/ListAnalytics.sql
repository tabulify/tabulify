...
SET list_analytics      =
      coalesce(list_analytics, '{}'::jsonb) || ((
        ...
        ' "userOutCount": ' || analytics.userOutListCount::text ||
        '}')::jsonb),
...
from (select realm_list.list_id,
             ...
             coalesce(userCount.userOutListCount, 0) as userOutListCount
      from realm_list
             LEFT OUTER JOIN (select list_user_realm_id
                                   , list_user_list_id
                                   , count(*)                                          as userCount
                                   , COUNT(CASE WHEN list_user_status = 0 THEN 1 END)  AS userInListCount
                                   , COUNT(CASE WHEN list_user_status != 0 THEN 1 END) AS userOutListCount
