...
import net.bytle.tower.eraldy.model.openapi.AnalyticsEventContext;
...
public class AnalyticsLogger {
  ...
  static public void log(AnalyticsEvent analyticsEvent, RoutingContext routingContext) {
    ...
    // Realm
    // TODO: The app should be mandatory
    Realm authRealm = routingContextWrapper.getAuthRealm();
    // to avoid recursion when we serialize in json, we clone
    authRealm = RealmProvider.createFrom(routingContext.vertx()).toPublicClone(authRealm);
    context.setRealm(authRealm);
    ...
    ThreadContext.put(LOOKUP_VARIABLE_REALM_GUID, analyticsEvent.getContext().getRealm().getGuid());
