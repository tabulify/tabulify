...
import net.bytle.tower.eraldy.model.openapi.AnalyticsEventChannel;
import net.bytle.tower.eraldy.model.openapi.AnalyticsOperatingSystem;
...
public class AnalyticsMixPanel {
   ...
   */
  public static JSONObject toMixPanelUser(User user, String ip) {
    ...
    try {
      props.put("$name", UsersUtil.getNameOrNameFromEmail(user));
  ...
  protected static JSONObject toMixpanelPropsWithoutUserId(AnalyticsEvent event) {
    ...
    if (user != null) {
      props.put("$user_id", event.getUser().getGuid());
    }
