...
public class EmailLoginFlow extends WebFlowAbs {
  ...
  public Future<ApiResponse<Void>> handleStep1SendEmail(RoutingContext routingContext, User modelUserToLogin, UriEnhanced redirectUri) {
    ...
    AuthJwtClaims jwtClaims = getApp()
      ...
      .setAudienceOrganizationGuid(jackson.getSerializer(OrgaGuid.class).serialize(requestingApp.getRealm().getOrganization().getGuid()))
      .setAudienceOrganizationHandle(jackson.getSerializer(Handle.class).serialize(requestingApp.getRealm().getOrganization().getHandle()))
      .build()
      .toJwtClaims()
