...
//          ));
  ...
  public Future<ApiResponse<Void>> analyticsEventPost(RoutingContext routingContext, Map<String, Object> requestBody) {
    ...
    analyticsClient.setAppOrganisationGuid(jackson.getSerializer(OrgaGuid.class).serialize(authRealm.getOwnerUser().getOrganization().getGuid()));
    analyticsClient.setAppOrganisationHandle(jackson.getSerializer(Handle.class).serialize(authRealm.getOwnerUser().getOrganization().getHandle()));
