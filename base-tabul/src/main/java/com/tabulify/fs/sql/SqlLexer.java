...
final public class SqlLexer {
  ...
  public List<SqlStatement> parse(Path path, Charset charset) {
    TextFile.FsTextFileBuilder fsTextFileBuilder = TextFile.builder(path).setCharset(charset);
    TextFile fileText;
    try {
  ...
  public static class SqlPlusLexerBuilder {
    ...
    // The `;` is oracle,
    private Set<String> endOfLineEndCommandCharacterSet = Set.of(";");

    // The characters that end a pl sql statement at the end of a line
    // the // and | comes from example in MySQL set by the DELIMITER instruction
    // $$; is postgres
    // end; is correct mysql
    private Set<String> endOfLineEndCommandPsqlCharacterSet = new HashSet<>(List.of("//", "$$;", "end;"));

    public Set<String> getEndOfLineEndCommandPsqlCharacterSet() {
      return endOfLineEndCommandPsqlCharacterSet;
    }

    public void setEndOfLineEndCommandPsqlCharacterSet(Set<String> endOfLineEndCommandPsqlCharacterSet) {
      this.endOfLineEndCommandPsqlCharacterSet = endOfLineEndCommandPsqlCharacterSet;
    }
