...
public class ManifestKindManager implements FsFileManager {
    ...
    public DataPath createDataPath(FsConnection fsConnection, Path relativePath, MediaType mediaType) {
        ...
        try {
            ...
            DataUriNode dataUriNode = DataUriBuilder
            ...
            /*
             * For a static resource, if the data uri is a runtime uri, we execute it to get a static resource
             */
            if (!this.targetMediaType.isRuntime() && dataUriNode.isRuntime()) {
                dataUriNode = fsConnection
                        .getTabular()
                        .getDataPath(dataUriNode, null)
                        .execute()
                        .toDataUri();
            }

            /*
             * If the data uri is a manifest path, we execute it
             */
